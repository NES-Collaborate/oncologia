...
def create_fake_data_from_df(df):
    with app.app_context():
        ...
        for _, row in df.iterrows():
            ...
            new_entry_team = EntryTeam.query.filter_by(
                name=row["Equipe de Entrada"]
            ).first()
            if not new_entry_team:
                new_entry_team = EntryTeam(name=row["Equipe de Entrada"])
            new_entry_poin = EntryPoin.query.filter_by(
                name=row["Porta de Entrada"]
            ).first()
            if not new_entry_poin:
                new_entry_poin = EntryPoin(name=row["Porta de Entrada"])

            new_tumor_group = TumorGroup.query.filter_by(
                name=row["GRUPO DE TUMOR"]
            ).first()
            if not new_tumor_group:
                new_tumor_group = TumorGroup(name=row["GRUPO DE TUMOR"])

            new_exam_type = ExamType.query.filter_by(
